import { animation, keyframes, state, stagger, trigger, animateChild, group, transition, animate, style, query, AnimationAnimateMetadata } from '@angular/animations';
...
function animCombiner(speed: string|number, timing: string|number, style?:any): AnimationAnimateMetadata{
...
export const ShowHide = trigger('ShowHide', [
	...
	transition(':leave', [
		query('@*', animateChild(), { optional: true }),
		animCombiner(speed.fast, timing.easeOut, style(styles.hide))
	])
]);
...
export const PickerContent = trigger('PickerContent', [
	...
	transition('* => from-top', [
		...
		animCombiner(speed.fast, timing.easeIn)
	]),
	transition('from-top => void', animCombiner(
		speed.fast, timing.easeOut,
		style(styles.X0Y100_N)
	)),
	// From bottom
