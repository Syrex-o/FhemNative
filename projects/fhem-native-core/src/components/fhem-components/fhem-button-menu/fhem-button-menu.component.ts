...
export class FhemButtonMenuComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		if(this.data_device){
			this.fhem.getDevice(this.ID, this.data_device, (device: FhemDevice)=>{
				...
				// attatch label
				if(device){
					this.data_label = (this.data_label && this.data_label !== '') ? this.data_label : device.device;
				}
			});
		}
	...
	switchRoom(): void{
		const neededRoom: Room|undefined = this.structure.rooms.find(x=> x.name === this.data_room);
		if(neededRoom){
...
@NgModule({
	imports: [FhemComponentModule, IconModule, TranslateModule, IonicModule],
