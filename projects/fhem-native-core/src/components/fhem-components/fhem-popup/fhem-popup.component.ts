...
export class FhemPopupComponent implements OnInit, OnDestroy {
	...
	ngOnInit(){
		if(this.data_device){
			this.fhem.getDevice(this.ID, this.data_device, (device: FhemDevice|null)=>{
	...
	popupClosed(){
		...
		const popupComponents: DynamicComponentDefinition[]|null = this.structure.getComponentContainer(this.container);
		if(popupComponents){
			popupComponents.forEach((component: DynamicComponentDefinition)=>{
