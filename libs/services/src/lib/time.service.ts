...
export class TimeService {
	...
	public times(time: string): {hours: number, mins: number, toMin: number} {
	...
	public secToTime(sec: any): string{
		const t: any = parseFloat(sec).toFixed(3);
		const h: any = Math.floor(t / 60 / 60);
		const m: any = Math.floor(t / 60) % 60;
		const s: any = Math.floor(t - m * 60);
	...
	public duration(start: number|string, end: number|string): {min: number, time: string} {
