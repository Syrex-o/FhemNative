...
export class FhemButtonMultistateComponent implements OnInit, OnDestroy {
	...
	private initStates(): void{
		...
		this.data_label = (this.data_label && this.data_label !== '') ? this.data_label : (this.fhemDevice ? this.fhemDevice.device : '');
	}
	...
	getArrValue(arr: string, def: string|boolean): string{
		...
		if(this.fhemDevice && (this.data_reading in this.fhemDevice.readings)){
			...
			const getList: string[] = this.data_getOn.map((x: any)=> x.toString().toLowerCase());
...
@NgModule({
	imports: [FhemComponentModule, IconModule, IonicModule],
